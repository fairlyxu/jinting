...
(function (root) {
    ...
    TextNode.prototype = {
        ...
        getRendererBody: function () {
            ...
            var variableOpen = (typeof this.engine.options.variableOpen === 'undefined') ? '${' :
                this.engine.options.variableOpen;
            var variableClose = (typeof this.engine.options.variableClose === 'undefined') ? '}' :
                this.engine.options.variableClose;
            var code = [];
            parseTextBlock(
                this.value, variableOpen, variableClose, 1,
                ...
                for (var i = 0, len = masterNode.children.length; i < len; i++) {
                    ...
                    if (child instanceof ContentPlaceHolderCommand) {
                        this.children.push.apply(
                            this.children, (this.contents[child.name] || child).children
                        );
                    } else {
    ...
    TargetCommand.prototype.isReady = function () {
        ...
        function checkReadyState(node) {
            for (var i = 0, len = node.children.length; i < len; i++) {
                ...
                if (child instanceof ImportCommand || child instanceof UseCommand) {
    ...
    ForCommand.prototype.getRendererBody = function () {
        return stringFormat(
            '' + 'var {0}={1};' + 'if({0} instanceof Array)' + 'for (var {4}=0,{5}={0}.length;{4}<{5};{4}++){v[{2}]={4};v[{3}]={0}[{4}];{6}}' + 'else if(typeof {0}==="object")' + 'for(var {4} in {0}){v[{2}]={4};v[{3}]={0}[{4}];{6}}',
            generateGUID(),
